...
class List_BaseRepair extends BaseModel {

    public function repairList($event){
        ...
        if (isset($data["phone"]) && !empty($data["phone"])) {
            $condition .= " AND bp.phone = :phone";
            $params[":phone"] = $data["phone"];
        }
        if (isset($data["contacts"]) && !empty($data["contacts"])) {
            $condition .= " AND bp.contacts = :contacts";
            $params[":contacts"] = $data["contacts"];
        }
        if (isset($data["handler"]) && !empty($data["handler"])) {
            $condition .= " AND bp.handler = :handler";
            $params[":handler"] = $data["handler"];
        }
        if (isset($data["status"]) && !empty($data["status"])) {
            $condition .= " AND bp.status = :status";
            $params[":status"] = $data["status"];
        }
        $View_BaseProduct = new View_BaseRapair();
        $dataAttr = $View_BaseProduct->getAllRepair($event, $ispage, $condition, $params, $limit);
